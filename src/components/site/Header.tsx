...
import { PainelLoginModal } from "@/components/site/PainelLoginModal";
...
export function Header({ painelLoginOpen, onPainelLoginOpenChange }: HeaderProps) {
  const navigate = useNavigate();
  const [open, setOpen] = useState(false);
  const [localLoginOpen, setLocalLoginOpen] = useState(false);
  const [checkingPainel, setCheckingPainel] = useState(false);

  const loginOpen = painelLoginOpen ?? localLoginOpen;
  const setLoginOpen = onPainelLoginOpenChange ?? setLocalLoginOpen;
  ...
  return (
    <>
        ...
        <div className="mx-auto max-w-7xl h-full px-6 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-4">
            <button
              ...
              className="text-sm font-medium text-brand-muted hover:text-brand-navy disabled:opacity-60"
            >
              {checkingPainel ? "Abrindo…" : "Painel"}
            </button>
          ...
          <div id="mobile-menu" className="lg:hidden border-t border-brand-border bg-white">
            <nav className="px-6 py-4 flex flex-col gap-3" aria-label="Navegação móvel">
              ...
              <button
                ...
                onClick={() => void abrirPainel()}
                disabled={checkingPainel}
                className="py-2 text-left text-brand-muted disabled:opacity-60"
              >
                {checkingPainel ? "Abrindo…" : "Painel"}
              </button>
      ...
      <PainelLoginModal open={loginOpen} onClose={fecharLogin} />
    </>
  );
}
