...
import { obterFornecedorPreviewId, usuarioPodeSimularVisao } from "@/lib/visao.server";
...
export async function buildAdminSessionPayload(user: {
  ...
  if (v === "fornecedor") {
    fornecedorPreviewId = await obterFornecedorPreviewId();
  }
  return {
    userId: user.id,
    nome: user.nome,
...
/** Login com tabela `usuarios` (senha bcrypt PHP $2y$). Apenas nível admin. */
export async function authenticateUsuario(
  ...
  const row = await prisma.usuario.findFirst({
    where: { usuario: login.trim() },
  });
  if (!row) return null;

  const nivel = row.nivel?.toLowerCase() ?? "";
  if (nivel !== "admin") return null;

  const bcrypt = await import("bcryptjs");
  const hash = row.senha.replace(/^\$2y\$/, "$2a$");
...
function visaoPorNivel(nivel: string): AdminVisao {
...
export function redirectAposImpersonacao(
  ...
  const v = visaoPorNivel(nivel);
  if (v === "funcionário") return "/painel/ponto";
  if (v === "fornecedor") {
...
export async function iniciarImpersonacao(
  ...
  const payload: Omit<AdminSessionPayload, "exp"> = {
    ...
    visao: session.podeSimular ? visao : "admin",
    podeSimular: session.podeSimular,
    fornecedorPreviewId,
